...
class CandidateProfileForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = ('first_name', 'last_name', 'address_line', 'zip_code', 'state', 'email', 'portfolio_website',
                  'education', 'experiences', 'cover_letter', 'gender', 'ethnicity', 'race', 'health_conditions',
                  'veteran')

    def __init__(self, *args, **kwargs):
        candidate = kwargs.pop('instance')
        super(CandidateProfileForm, self).__init__(*args, **kwargs)
        # Initialize the form so it contains the information the user has already provided
        # Make sure the first letter of the user's first and last name are capitalize
        self.fields['first_name'].initial = candidate.candidate_profile.first_name[:1].upper() \
                                            + candidate.candidate_profile.first_name[1:]
        self.fields['last_name'].initial = candidate.candidate_profile.last_name[:1].upper() \
        ...
        self.fields['portfolio_website'].initial = candidate.candidate_profile.portfolio_website
        ...
        self.fields['health_conditions'].initial = candidate.candidate_profile.health_conditions
